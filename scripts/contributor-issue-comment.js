...
function formatAssignAtLimitMessage(assignedIssues, recentUnassignments) {
  ...
  if (recentUnassignments.length > 0) {
    ...
    const cooldownMs = ASSIGN_COOLDOWN_DAYS * msPerDay;
    message += '\n\n**Recently dropped issues (cooldown):**\n';
    message += recentUnassignments
      .map(u => {
        const daysRemaining = Math.ceil(
          (cooldownMs - (Date.now() - new Date(u.unassignedAt).getTime())) / msPerDay,
        );
...
module.exports = async ({ github, context, core }) => {
  try {
    ...
    const isAssignCommand = /\/assign\b/i.test(commentBody);
    if (isAssignCommand) {
